...
public class UndoController {
    ...
    public void undoCallback() {
        if (undoBuffer.isPresent()) {
            determineLogicMethod(undoBuffer.get().getKey(), undoBuffer.get().getValue());
            undoBuffer = Optional.empty();
        }
    }
    ...
    public void addAction(TurboIssue issue, Action action) {
        determineLogicMethod(issue, action);
        undoBuffer = Optional.of(new Pair<>((TurboIssue) action.act(issue), action.invert()));
    ...
    // Check to determine which logic method to call
    @SuppressWarnings("unchecked")
    private void determineLogicMethod(TurboIssue issue, Action action) {
        if (action.getType() == Action.ActionType.CHANGELABELS) {
            logic.replaceIssueLabels(issue, action)
                    .thenApply(success -> showErrorDialogOnFailure((Boolean) success, issue, action));
        }
    }
